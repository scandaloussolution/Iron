...
local phantom_forces_modules = _G.modules_pf or {
	["character"] = findLocal({
		"setsprint",
		"setbasaewalkspeed",
		"getstate",
...
_G.modules_pf = phantom_forces_modules

if phantom_forces_modules.character then
	local setbasewalkspeed = phantom_forces_modules.character.setbasewalkspeed
	function phantom_forces_modules.character:setbasewalkspeed(speed, ...)
		return setbasewalkspeed(self, _G.ws or 16, ...)
	end
end
					...
					["NewSlider"] = {
						{
							args = {
								...
								100,
								function(val)
									if char and not name:lower():find("phantom forces") then
										local h = char:FindFirstChildOfClass("Humanoid")
										if h then
											_G.ws = val
											h.WalkSpeed = val
